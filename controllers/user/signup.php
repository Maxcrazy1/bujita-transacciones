...
if(isset($post['submit_form'])) {
	...
	if($post['name'] && $post['email'] && $post['phone']) {
		...
		$name = real_escape_string($post['name']);//real_escape_string($post['first_name']).' '.real_escape_string($post['last_name']);
		...
		if(!empty($get_userdata_row) && $get_userdata_row['status']=='0') {
			...
			if($is_verification) {
    		    ...
    		    $header_logo = str_replace('my_preheader', substr(preg_replace('/\{\$\w+\_\w+\}/i',"", preg_replace('/\{\$(customer_fullname)\}(\W)/i', $temp.arr."$2".' ' ,$template_data_for_email['content'])),0,300), $header_logo);
			...
			$query=mysqli_query($db,"INSERT INTO `users`(`name`, `first_name`, `last_name`, `phone`, `email`, `address`, `address2`, `city`, `state`, `postcode`, `username`, `password`, `status`, `date`, verification_type, verification_code,`is_news_letter_subscribed`) VALUES('".$name."', '".real_escape_string($post['first_name'])."','".real_escape_string($post['last_name'])."','".$phone."','".$email."','".real_escape_string($post['address'])."','".real_escape_string($post['address2'])."','".real_escape_string($post['city'])."','".real_escape_string($post['state'])."','".real_escape_string($post['postcode'])."','".$email."','".$password."','".$status."','".date('Y-m-d H:i:s')."','".$verification_type."','".$verification_code."','".$agree."')");
			if($query=="1") {
				if($is_verification) {
    		    ...
    		    $header_logo = str_replace('my_preheader', substr(preg_replace('/\{\$\w+\_\w+\}/i',"", preg_replace('/\{\$(customer_fullname)\}(\W)/i', $temp_arr."$2".' ' ,$template_data_for_email['content'])),0,300), $header_logo);
