...
if(isset($post['submit_form'])) {
	...
	if($name=="" || $email=="" || $phone=="") {
		$msg='Please fillup mandatory fields.';
		setRedirectWithMsg(SITE_URL.'enterdetails',$msg,'danger');
		exit();
	}
	...
	if(!empty($get_userdata_row) && $get_userdata_row['status']=='1') {
		...
		//Added new logic
		$msg='This account is already exist so you can please <a href="'.$login_link.'" class="btn-sm btn-primary">login</a> with continue';
		setRedirectWithMsg(SITE_URL.'enterdetails',$msg,'warning');
		exit();
