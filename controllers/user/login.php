...
if(isset($post['submit_form'])) {
	...
	if($checkUser > 0 && $fetch_userdata['status'] == '0') {
	 	$msg='<?php echo $LANG["Sorry, your email address has not been verified yet. To re-send verification use the sign-up form."]; ?>';
		setRedirectWithMsg($return_url,$msg,'warning');
		exit();
		...
		$msg = '<?php echo $LANG["YOU HAVE SUCCESSFULLY LOGGED IN"]; ?>';
		...
		if($order_id!="") {
			...
			if($basket_item_count_sum_data['basket_item_count']>0) {
				...
				if($order_data['paypal_address']!="" || $order_data['chk_name']!="" || $order_data['act_name']!="") {
					$redirect_url = SITE_URL.'revieworder?action=confirm';
				} else {
		...
		$_SESSION['user_prefill_email']=$post['username'];
		$msg='<?php echo $LANG["Please enter correct login details"]; ?>';
		setRedirectWithMsg($return_url,$msg,'warning');
		exit();
    }
} else {
