...
if (!empty($screen_size_list)) {
	$n_i = $n_i + 1;
	$n_i_array['screen_size'] = $n_i . '.';
}
if (!empty($screen_resolution_list)) {
	$n_i = $n_i + 1;
	$n_i_array['screen_resolution'] = $n_i . '.';
}
if (!empty($lyear_list)) {
	$n_i = $n_i + 1;
	$n_i_array['lyear'] = $n_i . '.';
}
if (!empty($processor_list)) {
	$n_i = $n_i + 1;
	$n_i_array['processor'] = $n_i . '.';
}
...
<script>
	...
	$(document).ready(function() {
		...
		$("#nextBtn").click(function() {
			...
			if (current_id == "screen_size") {
				if (!$("input[name='check_screen_size']").is(':checked')) {
					$("#screen_size_error_msg").show().text('Please select a screen size');
					return false;
				}
			}
			if (current_id == "screen_resolution") {
				if (!$("input[name='check_screen_resolution']").is(':checked')) {
					$("#screen_resolution_error_msg").show().text('Please select a screen resolution');
					return false;
				}
			}
			if (current_id == "lyear") {
				if (!$("input[name='check_lyear']").is(':checked')) {
					$("#lyear_error_msg").show().text('Please select a year');
					return false;
				}
			}
			if (current_id == "processor") {
				if (!$("input[name='check_processor']").is(':checked')) {
					$("#processor_error_msg").show().text('Please select a processor');
					return false;
				}
			}
			...
			if (current_id == "ram") {
				if (!$("input[name='check_ram']").is(':checked')) {
					$("#ram_error_msg").show().text('Please select a ram size');
					return false;
				}
			}
			if (current_id == "color") {
		...
		$(".check_screen_size").click(function() {
			$("#screen_size_error_msg").hide();
			network_cal('screen_size');
		});

		$(".check_screen_resolution").click(function() {
			$("#screen_resolution_error_msg").hide();
			network_cal('screen_resolution');
		});

		$(".check_lyear").click(function() {
			$("#lyear_error_msg").hide();
			network_cal('lyear');
		});

		$(".check_processor").click(function() {
			$("#processor_error_msg").hide();
	...
	function network_cal(type) {
		jQuery(document).ready(function($) {
            ...
            if(sessionStorageSelected){
				...
				screen_size_id=sessionStorageSelected.screen_size_id;
				screen_resolution_id=sessionStorageSelected.screen_resolution_id;
				lyear_id=sessionStorageSelected.lyear_id;
				processor_id=sessionStorageSelected.processor_id;
				ram_id=sessionStorageSelected.ram_id;
			...
			assigned_dependencies['processor'] = processor_id;
			assigned_dependencies['ram'] = ram_id;
			assigned_dependencies['screen_resolution'] = screen_resolution_id;
			assigned_dependencies['screen_size'] = screen_size_id;
			assigned_dependencies['lyear'] = lyear_id;			
...
<div class="col-md-8">
	...
	<div class="step-details clearfix">
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Type Terms & Conditions']; ?></h5>
			...
			<div id="network" class="block phone-details network m-0 p-0 clearfix">
				...
				<div class="network_price_mode_section_showhide maxkyu-modified" style="display:none;">
					...
					<div class="row info-section pt-5 network_price_lock_warning_showhide" style="display:none;">
						...
						<div class="col-10">
							<!-- <p class="col-10-pad">A third-party unlocking fee ($50 - $150) will be deducted from your final offer. No offer will be made if the unlocking fee exceeds the value of your device. Unlock your device to the receive the best offer!</p> -->
				...
				<div class="modal fade" id="NetworkHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Network Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
		...
		if (!empty($screen_size_list)) { ?>
			<div id="screen_size" class="block phone-details storage m-0 p-0 clearfix">
				<h4 class="h4"><?= $n_i_array['screen_size'] ?> <?= $screen_size_title ?></h4>
				<div class="clearfix pb-3">
					<?php
					foreach ($screen_size_list as $ss_key => $screen_size_data) { ?>
						<div class="custom-control custom-radio" id="depen_screen_size<?= $screen_size['id']?>">
							<input type="radio" id="check_screen_size<?= $ss_key ?>" name="check_screen_size" value="<?= $screen_size_data['id'] ?>" <?php if ($order_item_data['screen_size'] != "" && $order_item_data['screen_size'] == $screen_size_data['screen_size_name']) {
																																																																					echo 'checked="checked"';
																																																																				} ?> class="custom-control-input check_screen_size">
							<label class="slider-v2" for="check_screen_size<?= $ss_key ?>" onclick=""></label>
							<label class="custom-control-label" for="check_screen_size<?= $ss_key ?>"><?= $screen_size_data['screen_size_name'] ?></label>
						</div>
					<?php
					} ?>
				</div>
				<div class="clearfix">
					<small id="screen_size_error_msg" class="help-block m_validations_showhide" style="display:none;"></small>
				</div>
				<a class="suppot-ques" href="javascript:void();" data-toggle="modal" data-target="#ScreenSizeHelp"><?php echo $LANG["Not sure? Here's how to check"]; ?></a>

				<div class="modal fade" id="ScreenSizeHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Screen Size Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<?= $category_data['tooltip_screen_size'] ?>
							</div>
						</div>
					</div>
				</div>
		...
		if (!empty($screen_resolution_list)) { ?>
			<div id="screen_resolution" class="block phone-details storage m-0 p-0 clearfix">
				<h4 class="h4"><?= $n_i_array['screen_resolution'] ?> <?= $screen_resolution_title ?></h4>
				<div class="clearfix pb-3">
					<?php
					foreach ($screen_resolution_list as $sr_key => $screen_resolution_data) { ?>
						<div class="custom-control custom-radio" id="depen_screen_resolution<?= $screen_resolution_data['id']?>">
							<input type="radio" id="check_screen_resolution<?= $sr_key ?>" name="check_screen_resolution" value="<?= $screen_resolution_data['id'] ?>" <?php if ($order_item_data['screen_resolution'] != "" && $order_item_data['screen_resolution'] == $screen_resolution_data['screen_resolution_name']) {
																																																																														echo 'checked="checked"';
																																																																													} ?> class="custom-control-input check_screen_resolution">
							<label class="slider-v2" for="check_screen_resolution<?= $sr_key ?>" onclick=""></label>
							<label class="custom-control-label" for="check_screen_resolution<?= $sr_key ?>"><?= $screen_resolution_data['screen_resolution_name'] ?></label>
						</div>
					<?php
					} ?>
				</div>
				<div class="clearfix">
					<small id="screen_resolution_error_msg" class="help-block m_validations_showhide" style="display:none;"></small>
				</div>
				<a class="suppot-ques" href="javascript:void();" data-toggle="modal" data-target="#ScreenResolutionHelp"><?php echo $LANG["Not sure? Here's how to check"]; ?></a>

				<div class="modal fade" id="ScreenResolutionHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Screen Resolution Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<?= $category_data['tooltip_screen_resolution'] ?>
							</div>
						</div>
					</div>
				</div>
		...
		if (!empty($lyear_list)) { ?>
			<div id="lyear" class="block phone-details storage m-0 p-0 clearfix">
				<h4 class="h4"><?= $n_i_array['lyear'] ?> <?= $lyear_title ?></h4>
				<div class="clearfix pb-3">
					<?php
					foreach ($lyear_list as $ly_key => $lyear_data) { ?>
						<div class="custom-control custom-radio" id="depen_lyear<?= $lyear_data['id']?>">
							<input type="radio" id="check_lyear<?= $ly_key ?>" name="check_lyear" value="<?= $lyear_data['id'] ?>" <?php if ($order_item_data['lyear'] != "" && $order_item_data['lyear'] == $lyear_data['lyear_name']) {
																																																												echo 'checked="checked"';
																																																											} ?> class="custom-control-input check_lyear">
							<label class="slider-v2" for="check_lyear<?= $ly_key ?>" onclick=""></label>
							<label class="custom-control-label" for="check_lyear<?= $ly_key ?>"><?= $lyear_data['lyear_name'] ?></label>
						</div>
					<?php
					} ?>
				</div>
				<div class="clearfix">
					<small id="lyear_error_msg" class="help-block m_validations_showhide" style="display:none;"></small>
				</div>
				<a class="suppot-ques" href="javascript:void();" data-toggle="modal" data-target="#YearHelp"><?php echo $LANG["Not sure? Here's how to check"]; ?></a>
				<div class="modal fade" id="YearHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Year Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<?= $category_data['tooltip_lyear'] ?>
							</div>
						</div>
					</div>
				</div>
		...
		if (!empty($processor_list)) { ?>
			<div id="processor" class="block phone-details storage m-0 p-0 clearfix">
				<h4 class="h4"><?= $n_i_array['processor'] ?> <?= $processor_title ?></h4>
				<div class="clearfix pb-3">
					<?php
					foreach ($processor_list as $prcr_key => $processor_data) { ?>
						<div class="custom-control custom-radio" id="depen_processor<?= $processor_data['id']?>">
							<input type="radio" id="check_processor<?= $prcr_key ?>" name="check_processor" value="<?= $processor_data['id'] ?>" <?php if ($order_item_data['processor'] != "" && $order_item_data['processor'] == $processor_data['processor_name']) {
																																																																			echo 'checked="checked"';
																																																																		} ?> class="custom-control-input check_processor">
							<label class="slider-v2" for="check_processor<?= $prcr_key ?>" onclick=""></label>
							<label class="custom-control-label" for="check_processor<?= $prcr_key ?>"><?= $processor_data['processor_name'] ?></label>
						</div>
					<?php
					} ?>
				</div>
				<div class="clearfix">
					<small id="processor_error_msg" class="help-block m_validations_showhide" style="display:none;"></small>
				</div>
				<a class="suppot-ques" href="javascript:void();" data-toggle="modal" data-target="#ProcessorHelp"><?php echo $LANG["Not sure? Here's how to check"]; ?></a>

				<div class="modal fade" id="ProcessorHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Processor Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<?= $category_data['tooltip_processor'] ?>
							</div>
						</div>
					</div>
				</div>
			...
			<div id="storage" class="block phone-details storage m-0 p-0 clearfix">
				...
				<div class="modal fade" id="CapacityHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><?php echo $LANG['Capacity Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
		...
		if (!empty($ram_list)) { ?>
			<div id="ram" class="block phone-details storage m-0 p-0 clearfix">
				<h4 class="h4"><?= $n_i_array['ram'] ?> <?= $ram_title ?></h4>
				<div class="clearfix pb-3">
					<?php
					foreach ($ram_list as $rm_key => $ram_data) {
						$s_ram_size = $ram_data['ram_size'] . $ram_data['ram_size_postfix']; ?>
						<div class="custom-control custom-radio"  id="depen_ram<?= $ram_data['id']?>">
							<input type="radio" id="check_ram<?= $rm_key ?>" name="check_ram" value="<?= $ram_data['id'] ?>" <?php if ($order_item_data['ram'] != "" && $order_item_data['ram'] == $s_ram_size) {
																																																									echo 'checked="checked"';
																																																								} ?> class="custom-control-input check_ram">
							<label class="slider-v2" for="check_ram<?= $rm_key ?>" onclick=""></label>
							<label class="custom-control-label" for="check_ram<?= $rm_key ?>"><?= $s_ram_size ?></label>
						</div>
					<?php
					} ?>
				</div>
				<div class="clearfix">
					<small id="ram_error_msg" class="help-block m_validations_showhide" style="display:none;"></small>
				</div>
				<a class="suppot-ques" href="javascript:void();" data-toggle="modal" data-target="#RamHelp"><?php echo $LANG["Not sure? Here's how to check"]; ?></a>

				<div class="modal fade" id="RamHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id=""><?php echo $LANG['Ram (Memory) Terms & Conditions']; ?></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body">
								<?= $category_data['tooltip_ram'] ?>
							</div>
						</div>
					</div>
				</div>
