...
<script>
	...
	$(document).ready(function() {
		...
		$("#nextBtn").click(function() {
			...
			if (current_id == "storage") {
				...
				// var storage_color_ids = $("input[name='check_capacity']:checked").attr("data-storage_color_ids");
				// if(storage_color_ids) {
				// 	$(".color_item").hide();
				// 	$('input:radio[name=check_color]').each(function() {
				// 		$(this).prop('checked', false);
				// 	});

				// 	var storage_color_ids_arr = storage_color_ids.split(",");
				// 	$.each(storage_color_ids_arr,function(i) {
				// 	   $("#color_item-"+storage_color_ids_arr[i]).show();
				// 	});
				// } else {
				// 	$(".color_item").show();
				// }
	...
	function network_cal(type) {
		jQuery(document).ready(function($) {
            ...
            if(sessionStorageSelected){
    			    ...
    			    email_state = sessionStorageSelected.email_state;
	...
	$(document).ready(function() {
				...
				current_dependency = $(".phone-details:visible").attr("id");
	...
	<div class="step-details clearfix">
		...
		if (!empty($network_list)) { ?>
			<div id="network" class="block phone-details network m-0 p-0 clearfix">
				...
				<div class="clearfix">
					<?php foreach ($network_list as $n_key => $network_data) { ?>
						<!-- Check if available with others -->

						<div class="custom-control custom-radio" id="depen_network<?= $network_data['id']?>">
							<input type="radio" name="check_network" class="custom-control-input check_network" id="<?= createSlug($network_data['network_name']) ?>" value="<?= $network_data['id'] ?>" <?php if ($order_item_data['network'] == $network_data['network_name']) { echo 'checked="checked"';} ?> data-nw_name="<?= strtolower($network_data['network_name']) ?>">
							<label class="slider-v2" for="<?= createSlug($network_data['network_name']) ?>" onclick=""></label>
                            <label class="custom-control-label" for="<?= createSlug($network_data['network_name']) ?>" onclick="relatedRadio('<?= createSlug($network_data['network_name']) ?>')">
							<?php if ($network_data['network_icon']) { ?>
				...
				<div class="clearfix"><small id="network_error_msg" class="help-block m_validations_showhide" style="display:none;"></small></div>
				<div class="network_price_mode_section_showhide  maxkyu-modified" style="display:none;">
					...
					<div class="row info-section pt-5 network_price_lock_warning_showhide" style="display:none;">
						...
						<div class="col-10">
							<!--<p>A third-party unlocking fee ($50 - $150) will be deducted from your final offer. 
							No offer will be made if the unlocking fee exceeds the value of your device. Unlock your device to the receive the best offer!</p> -->
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><strong><?php echo $LANG['DEVICE SIM LOCK STATUS']; ?></strong></h5></h5>
			...
			<div id="storage" class="block phone-details storage m-0 p-0 clearfix">
						...
						<div class="custom-control custom-radio" id="depen_storage<?= $storage_data['id']?>">
							<input type="radio" id="capacity<?= $s_storage_size ?>" name="check_capacity" value="<?= $storage_data['id'] ?>" <?php if ($order_item_data['storage'] != "" && $order_item_data['storage'] == $s_storage_size) {
                                     echo 'checked="checked"';
										 } ?> class="custom-control-input check_capacity" data-storage_color_ids="<?=$storage_data['color_ids']?>">
							<label class="slider-v2" for="capacity<?= $s_storage_size ?>" onclick="relatedRadio('<?= createSlug($network_data['network_name']) ?>')"></label> 
				...
				<div class="modal fade" id="CapacityHelp" tabindex="-1" role="dialog" aria-labelledby="notSureNwtwork" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel"><strong><?php echo $LANG['PHONE STORAGE CAPACITY']; ?></strong></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
		...
		if (!empty($color_list)) { ?>
			<div id="color" class="block phone-details phone-color m-0 p-0 clearfix maxkyu-modified">
				...
				foreach ($color_list as $cnc_key => $color_data) { ?>
					<div class="custom-control custom-radio color_item" id="depen_color<?= $color_data['id']?>">
