...
$order_p_query=mysqli_query($db,"SELECT COUNT(*) AS num_of_orders, o.*, u.first_name, u.last_name,u.name FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id WHERE o.status!='partial' AND o.is_trash='1' ".$filter_by." ORDER BY o.date DESC");
$order_p_data = mysqli_fetch_assoc($order_p_query);
$pages->set_total($order_p_data['num_of_orders']);
			
//Fetch list of order
$order_query=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name,u.name FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id WHERE o.status!='partial' AND o.is_trash='1' ".$filter_by." ORDER BY o.date DESC ".$pages->get_limit()."");

$order_paid_batch_q=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name,u.name FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id WHERE o.status!='partial' AND o.is_trash='1' GROUP BY o.payment_paid_batch_id ORDER BY o.date DESC");
