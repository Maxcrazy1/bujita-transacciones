...
if(isset($post['d_id'])) {
	...
	if($post['id']) {
		$query=mysqli_query($db,'UPDATE faqs_groups SET cat_id="'.$cat_id.'", title="'.$title.'", added_date="'.$date.'", status="'.$status.'" WHERE id="'.$post['id'].'"');
		if($query=="1") {
		...
		$query=mysqli_query($db,'INSERT INTO faqs_groups(cat_id, title, updated_date, status) values("'.$cat_id.'","'.$title.'","'.$date.'","'.$status.'")');
		if($query=="1") {
