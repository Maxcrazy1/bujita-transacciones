...
if(isset($post['d_id'])) {
	...
	$is_check_icloud = $post['is_check_icloud'];
	$id = $post['id'];

	//Check Valid SEF URL
	$is_valid_sef_url_arr = check_sef_url_validation($sef_url, $id, "brand");
	if($is_valid_sef_url_arr['valid']!=true) {
		$msg='This sef url already exist so please use other.';
		$_SESSION['error_msg']=$msg;
		if($id) {
			setRedirect(ADMIN_URL.'edit_brand.php?id='.$id);
		} else {
	...
	if($id) {
		$query=mysqli_query($db,'UPDATE brand SET title="'.$title.'", sef_url="'.$sef_url.'", meta_title="'.$meta_title.'", meta_desc="'.$meta_desc.'", meta_keywords="'.$meta_keywords.'" '.$imageupdate.', description="'.$description.'", is_check_icloud="'.$is_check_icloud.'", published="'.$published.'" WHERE id="'.$id.'"');
		if($query=="1") {
		...
		$query=mysqli_query($db,'INSERT INTO brand(title, sef_url, meta_title, meta_desc, meta_keywords, image, description, is_check_icloud, published) values("'.$title.'","'.$sef_url.'","'.$meta_title.'","'.$meta_desc.'","'.$meta_keywords.'","'.$image_name.'","'.$description.'","'.$is_check_icloud.'","'.$published.'")');
		if($query=="1") {
