...
if(isset($post['d_id'])) {	
	...
	if($post['id']) {
		$query=mysqli_query($db,'UPDATE reviews SET name="'.$name.'", email="'.$email.'", country="'.$country.'", state="'.$state.'", city="'.$city.'", stars="'.$stars.'", title="'.$title.'", content="'.$content.'", status="'.$status.'", website="'.$website.'", zip_code="'.$zip_code.'", device_sold="'.$device_sold.'"'.$imageupdate.' WHERE id="'.$post['id'].'"');
		if($query=="1") {
		...
		$query=mysqli_query($db,"INSERT INTO reviews(name, email, country, state, city, stars, title, content, date, status, photo, website, zip_code, device_sold) VALUES('".$name."','".$email."','".$country."','".$state."','".$city."','".$stars."','".$title."','".$content."','".date('Y-m-d H:i:s')."','".$status."','".$image_name."','".$website."','".$zip_code."','".$device_sold."')");
		if($query=="1") {
