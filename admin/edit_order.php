...
if($order_data_before_saved['promocode_id']>0 && $order_data_before_saved['promocode_amt']>0) {
	...
	$discount_amt_label = "Bonus Code:";
	if($order_data_before_saved['discount_type']=="percentage")
		$discount_amt_label = "Bonus Code (".$order_data_before_saved['discount']."% of Initial Quote):";
...
$comment_query=mysqli_query($db,"SELECT c.*, s.name AS staff_name, s.username AS staff_username FROM comments AS c LEFT JOIN admin AS s ON s.id=c.staff_id WHERE c.order_id='".$order_id."' ORDER BY c.id DESC");
$num_of_comment = mysqli_num_rows($comment_query);

$ims_query=mysqli_query($db,"SELECT * FROM inbox_mail_sms WHERE order_id='".$order_id."' ORDER BY id DESC");

if($admin_type=="super_admin") {
	$sg_query=mysqli_query($db,"SELECT * FROM staff_groups WHERE status='1' AND email!='' ORDER BY id ASC");
} else {
	if($display_department_specific_from_email_only_in_order == '1') {
		$sg_query=mysqli_query($db,"SELECT * FROM staff_groups WHERE status='1' AND email!='' AND id='".$loggedin_user_data['group_id']."'");
	} else {
		$sg_query=mysqli_query($db,"SELECT * FROM staff_groups WHERE status='1' AND email!='' ORDER BY id ASC");
	}
}
