            ...
            <article class="span12 data-block">
                <header>
					...
					<ul class="data-header-actions">
						<li><a href="edit_category.php">Add New</a></li>
					</ul>
					<?php
					} ?>
				</header>
                <section>
					...
					<div class="row-fluid">
						<div class="span4">
							...
							<form action="controllers/device_categories.php" method="POST" style="margin-bottom:0px;">
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
							...
							if($num_rows>0) {
								...
								<tr>
									<td><input type="checkbox" onclick="clickontoggle('<?=$category_data['id']?>');" class="sub_chk" name="chk[]" value="<?=$category_data['id']?>"></td>
									<td>
									<?php
									if($category_data['image'])
										echo '<img src="../images/categories/'.$category_data['image'].'" width="100" height="100" />';
									?>
									</td>
									<td><?=$category_data['title']?></td>
									<td><?=$category_data['fields_type']?></td>
									<td>
										<input type="text" class="input-small" id="ordering<?=$category_data['id']?>" value="<?=$category_data['ordering']?>" name="ordering[<?=$category_data['id']?>]">
									</td>
									<td>
										<?php
										if($prms_category_edit == '1') { ?>
										<a href="edit_category.php?id=<?=$category_data['id']?>" class="btn btn-alt btn-default"><i class="icon-pencil"></i></a>
										<?php
										}
										if($prms_category_delete == '1') { ?>
										<a href="controllers/device_categories.php?d_id=<?=$category_data['id']?>" class="btn btn-danger btn-alt" onclick="return confirm('are you sure to delete this record?')"><i class="icon-trash"></i></a> 
										<?php
										}
										if($category_data['published']==1) {
											echo '<a href="controllers/device_categories.php?p_id='.$category_data['id'].'&published=0"><button class="btn btn-alt btn-success" style="pointer-events: none;">Published</button></a>';
										} elseif($category_data['published']==0) {
											echo '<a href="controllers/device_categories.php?p_id='.$category_data['id'].'&published=1"><button class="btn btn-alt btn-danger" style="pointer-events: none;">Unpublished</button></a>';
										}
										?>
									</td>
								</tr>
