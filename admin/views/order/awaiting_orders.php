	...
	<section class="container" role="main">
         <div class="row">
            <article class="span12 data-block">
                <header><h2>Awaiting Orders</h2></header>
                <section>
					...
					<form method="post">
						<div class="control-group">
							<div class="controls">
								...
								<?php /*?><select name="status" id="status" class="span2">
								...
								if($post['filter_by']!="" || $post['filter_by_location']!="" || $post['status']!="" || $post['contractor_type']!="" || $post['from_date']!="" || $post['to_date']!="" || $post['is_payment_sent']!="" || $post['payment_method']!="") {
									echo '<a href="awaiting_orders.php"><button class="btn btn-alt btn-primary" type="button">Clear</button></a>';
								} ?>
							</div>
						</div>
					</form>
								...
								if($num_rows>0) {
									while($order_data=mysqli_fetch_assoc($order_query)) { 
										...
										<tr>
											...
											<td>
												<?php /*?><a href="order_offer.php?order_id=<?=$order_data['order_id']?>&order_mode=awaiting" class="btn btn-alt btn-default">Offer</a><?php */?>
												<?php
												if($prms_order_edit == '1') { ?>
												<a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=awaiting" class="btn btn-alt btn-default"><i class="icon-pencil"></i></a>
												<?php
												}
												if($prms_order_delete == '1') { ?>
												<a href="controllers/order/order.php?a_id=<?=$order_data['order_id']?>&order_mode=awaiting" class="btn btn-danger btn-alt" onclick="return confirm('Are you sure you want to archive this record?');"><i class="icon-trash"></i></a>
												<?php
												} ?>
											</td>
										</tr>
