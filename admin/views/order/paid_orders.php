	...
	<section class="container" role="main">
         <div class="row">
            <article class="span12 data-block">
                <header><h2>Paid Orders</h2></header>
                <section>
					...
					<form method="post">
						...
						<div class="control-group"><div class="controls">
								<select name="payment_paid_batch_id" id="payment_paid_batch_id" class="span2">
								...
								<?php
								if($post['filter_by']!="" || $post['filter_by_location']!="" || $post['status']!="" || $post['contractor_type']!="" || $post['from_date']!="" || $post['to_date']!="" || $post['is_payment_sent']!="" || $post['payment_paid_batch_id']!="" || $post['payment_method']!="") {
									echo '<a href="paid_orders.php"><button class="btn btn-alt btn-primary" type="button">Clear</button></a>';
								} ?>
							</div>
						</div>
					</form>
							...
							<form action="controllers/order/order.php" method="POST" style="margin-bottom:0px;">
								<div class="control-group">
									<div class="controls">
										...
										<?php /*?><button class="btn btn-alt btn-danger bulk_set_paid" name="bulk_set_paid">Bulk Set Paid</button><?php */?>
									</div>	
								</div>
							</form>
							<?php
							...
							<form method="get" action="download_order_export.php" class="filter_form" style="margin-bottom:0px;text-align:right;">
								<div class="control-group">
									<div class="controls">
										...
										<button type="button" class="btn btn-alt btn-primary" name="import" onClick="ImportModal();return false;">Import Ref. Id</button>
										<button class="btn btn-alt btn-info" type="submit" name="export_ref_id_csv">Export Ref. Id (CSV)</button>
								...
								if($num_rows>0) {
									while($order_data=mysqli_fetch_assoc($order_query)) { 
										...
										<tr>
											...
											<?php /*?><td><?=($order_data['approved_date']!='0000-00-00 00:00:00'?date("m-d-Y",strtotime($order_data['approved_date'])):'')?></td><?php */?>
											...
											<td>
												<?php /*?><a href="order_offer.php?order_id=<?=$order_data['order_id']?>" class="btn btn-alt btn-default">Offer</a><?php */?>
												<?php
												if($prms_order_edit == '1') { ?>
												<a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=paid" class="btn btn-alt btn-default"><i class="icon-pencil"></i></a>
												<?php
												}
												if($prms_order_delete == '1') { ?>
												<a href="controllers/order/order.php?a_id=<?=$order_data['order_id']?>&order_mode=paid" class="btn btn-danger btn-alt" onclick="return confirm('Are you sure you want to archive this record?');"><i class="icon-trash"></i></a>
												<?php
												} ?>
											</td>
										</tr>
...
<div class="modal primary fade" id="import_modal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h3 class="modal-title">Import Ref. Id(s)</h3>
			</div>
			<!-- /Modal header -->

			<form action="controllers/order/order.php" method="POST" style="margin-bottom:0px;" enctype="multipart/form-data">
				<!-- Modal body -->
						...
						<div class="control-group">
							...
							<div class="controls">
								<div class="fileupload fileupload-new" data-provides="fileupload">
									...
									<br />
									<a href="sample_files/sample_ref_ids_file.csv" class="btn btn-alt btn-success"><i class="fa fa-download"></i> Download Sample File</a>
								</div>
							</div>
						</div>
					</fieldset>
				...
				<div class="modal-footer">
					<button type="submit" class="btn btn-alt btn-large btn-primary" name="import_ref_ids">IMPORT</button>
...
jQuery(document).ready(function($) {
	...
	$('.searchbx').on('click', function(e) {
		...
		if(val=="" && from_date=="" && to_date=="" && status=="" && is_payment_sent=="" && payment_paid_batch_id=="" && payment_method=="") {
			return false;
		}
	});
