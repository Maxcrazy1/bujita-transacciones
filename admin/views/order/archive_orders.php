	...
	<section class="container" role="main">
         <div class="row">
            <article class="span12 data-block">
                <header><h2>Archived Orders</h2></header>
                <section>
					...
					<form method="post">
						...
						<div class="control-group">
							<div class="controls">
								<select name="is_payment_sent" id="is_payment_sent" class="span2">
								...
								if($post['filter_by']!="" || $post['filter_by_location']!="" || $post['status']!="" || $post['contractor_type']!="" || $post['from_date']!="" || $post['to_date']!="" || $post['is_payment_sent']!="" || $post['payment_paid_batch_id']!="" || $post['payment_method']!="") {
									echo '<a href="archive_orders.php"><button class="btn btn-alt btn-primary" type="button">Clear</button></a>';
								} ?>
							</div>
						</div>
					</form>
							...
							<form action="controllers/order/order.php" method="POST" style="margin-bottom:0px;">
								<div class="control-group">
									<div class="controls">
										...
										<button class="btn btn-alt btn-success bulk_undo" name="bulk_undo">Bulk Undo</button>
									...
									while($order_data=mysqli_fetch_assoc($order_query)) { 
										...
										<tr>
											...
											<td>
												...
												<a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=archive" class="btn btn-alt btn-default"><i class="icon-pencil"></i></a>
												<?php
												}
												if($prms_order_delete == '1') { ?>
												<a href="controllers/order/order.php?d_id=<?=$order_data['order_id']?>" class="btn btn-danger btn-alt" onclick="return confirm('Are you sure you want to delete this record?');"><i class="icon-trash"></i></a>
												<a href="controllers/order/order.php?u_id=<?=$order_data['order_id']?>" class="btn btn-success btn-alt" onclick="return confirm('Are you sure you want to undone this record?');">Undo</a>
												<?php
												} ?>
											</td>
										</tr>
