	...
	<section class="container" role="main">
         <div class="row">
            <article class="span12 data-block">
                <header>
					<h2>Faqs Groups <?php /*?><a class="text-info" href="javascript:void(0);" onClick="showFaqInfo();return false;"><span class="elusive icon-info-sign"></span></a><?php */?></h2>
					<?php
					if($prms_faq_add == '1') { ?>
					<ul class="data-header-actions">
						<li><a href="edit_faq_group.php">Add New</a></li>
					</ul>
					<?php
					} ?>
				</header>
                <section>
					...
					<div class="row-fluid">
						<div class="span6">
							<div class="alert alert-gray fade in">
							   <strong>You can use this shortcode [faqs_groups] in pages for display group based faqs</strong>
							</div>
						</div>
					</div>
					...
					<div class="row-fluid">
						<div class="span4">
							<form action="controllers/faq_group.php" method="POST" style="margin-bottom:0px;">
							...
							if($num_rows>0) {
								...
								<tr>
									...
									<td>
										<?php
										if($prms_faq_edit == '1') { ?>
										<a href="edit_faq_group.php?id=<?=$faq_data['id']?>" class="btn btn-alt btn-default"><i class="icon-pencil"></i></a>
										<?php
										}
										if($prms_faq_delete == '1') { ?>
										<?php
										if($faq_data['type']!="fixed") { ?>
											<a href="controllers/faq_group.php?d_id=<?=$faq_data['id']?>" class="btn btn-danger btn-alt" onclick="return confirm('Are you sure to delete this record?')"><i class="icon-trash"></i></a>
										<?php
										}
										}
										if($faq_data['status']==1) {
											echo '<a href="controllers/faq_group.php?p_id='.$faq_data['id'].'&status=0"><button class="btn btn-alt btn-success" style="pointer-events: none;">Published</button></a>';
										} elseif($faq_data['status']==0) {
											echo '<a href="controllers/faq_group.php?p_id='.$faq_data['id'].'&status=1"><button class="btn btn-alt btn-danger" style="pointer-events: none;">Unpublished</button></a>';
										}
										?>
									</td>
								</tr>
