...
function get_single_model_data($req_model_id) {
	global $db;
	$response = array();
	$query=mysqli_query($db,"SELECT m.*, d.title AS device_title, d.sef_url, c.fields_type AS fields_cat_type FROM mobile AS m LEFT JOIN devices AS d ON d.id=m.device_id LEFT JOIN categories AS c ON c.id=m.cat_id WHERE m.id='".$req_model_id."'");
	$response = mysqli_fetch_assoc($query);
	return $response;
}

function get_device_brands_data_list($device_id) {
	global $db;
	$response = array();
...
function get_device_brand_models_data_list($device_id, $brand_id) {
	...
	$model_query=mysqli_query($db,"SELECT m.*, d.title AS device_title, d.sef_url AS device_sef_url, b.title AS brand_title, c.fields_type FROM mobile AS m LEFT JOIN devices AS d ON d.id=m.device_id LEFT JOIN brand AS b ON b.id=m.brand_id LEFT JOIN categories AS c ON c.id=m.cat_id WHERE m.published=1 ".$sql_whr." ORDER BY m.id DESC");
	$model_num_of_rows = mysqli_num_rows($model_query);
	if($model_num_of_rows>0) {
		while($model_list=mysqli_fetch_assoc($model_query)) {
			$response[] = $model_list;
