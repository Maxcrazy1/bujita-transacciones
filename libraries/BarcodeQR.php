...
final class BarcodeQR {
	...
	public function contact($codeContents = null) {
		$this->_data = $codeContents;
	}
	 ...
	 * @return bool
	 */
	public function draw($size, $filename = null, $margin) {
		if($size>'547')
		{
			$o_size=$size;
			$size='547';
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, "chs={$size}x{$size}&cht=qr&chld=".$EC_level."|".$margin.
"&chl=" . urlencode($this->_data));
		...
		if($img) {
			if($filename) {
				...
				if($margin=='0')
				{
					exec('convert '.$filename.' -fuzz 20% -trim -resize '.$o_size.'x'.$o_size.' '.$filename.'');
				}
				if($o_size>'547')
				{
					exec('convert '.$filename.' -resize '.$o_size.'x'.$o_size.' '.$filename.'');
				}
				return $filename;
			} else {
